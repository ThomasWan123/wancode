...
| **Work** | Local document understanding | Imports PDF, DOCX, XLSX, PPTX, and modern image files into a read-only staging area, builds verified model context, and keeps local extension capabilities disabled by default. |
...
- **Document Work surface** — one Add document entry for PDF, DOCX, XLSX, PPTX, PNG, JPEG, and WebP; durable recovery, verified read-only source identity, fail-closed source paths, bounded worker-process parsing, and citation-ready context supplied to every Work turn. Images require a vision-capable model, and image-only PDFs currently require external OCR.
