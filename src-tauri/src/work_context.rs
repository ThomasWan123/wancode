...
//!
//! Folder files are untrusted data. At send, the UI snapshots referenced
//! folder files into the existing read-only staging area; the model then
//! receives only text that has passed staged-file identity checks and the
//! crash-contained parser. A malformed manifest, replaced staged file,
//! unsupported kind, parser crash, or context overflow rejects the whole
//! turn instead of silently degrading to document-blind chat.
//!
//! An empty or missing snapshot (no @file / selected file this turn) passes
//! the user text through. Folder-first Work does not require a quarantined
//! import library.
...
pub fn build_work_context(
    ...
    if !manifest_path.exists() {
        ...
        // quarantined import library. Empty/missing snapshots pass the user
        // text through so @mentions and image-only waits still send.
    ...
    for record in &manifest.imports {
        ...
        let text = if let Some(kind) = parse_kind {
            ...
            total_image_bytes =
                reserve_image_bytes(&record.display_name, byte_len, total_image_bytes)?;
...
mod tests {
    ...
    fn image_extension_cannot_bypass_magic_validation() {
        ...
        let workspace = WorkspaceId::mint();
        let error = import_document(app.path(), &workspace, &source)
            .err()
            .map(|e| e.to_string())
            .unwrap_or_else(|| {
                build_work_context(app.path(), &workspace, "Inspect it")
                    .err()
                    .unwrap_or_default()
            });
