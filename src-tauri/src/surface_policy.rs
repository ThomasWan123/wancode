...
// W2.5：两个 agentProfile 构造器独立成 `surface_profiles.rs`——它们只依赖
// 外部 crate（无 `crate::` 引用），因而能被引擎探针用 `#[path]` 直接编进
// 测试 crate（见 tests/work_surface_engine.rs 与 job_breakaway 同款理由：
// 链接 wancode_lib 会因 dev profile 的 panic=abort 与测试目标的 unwind
// 冲突）。此处 re-export 保持既有调用点不变。
#[path = "surface_profiles.rs"]
mod surface_profiles;
pub use surface_profiles::{chat_agent_profile, work_agent_profile};
