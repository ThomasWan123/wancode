...
describe("engineDeadMessage", () => {
  ...
  it("translates 会话未启动 only after the backend confirmed engine death", () => {
    expect(engineDeadMessage("会话未启动", true, LABEL)).toBe(LABEL);
    // 正常态的「会话未启动」（从未启动过会话）不能被误翻成引擎退出。
    expect(engineDeadMessage("会话未启动", false, LABEL)).toBeNull();
  });
