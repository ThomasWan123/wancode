...
## 2026-08-02 #127 v0.18.9 兼容性治理五步收官

| PR | 内容 | 复核轮次 | 合并 commit |
|---|---|---|---|
| #7 | 能力模型与统一解析器（三态+来源+独立 wancode.toml+图片决策纯函数） | 6 轮 | e5dba21 |
| #8 | 能力快照与双数据链（配置世代+生产热加载+禁静默降级） | 3 轮 | fb090df |
| #9 | 能力徽章+图片路径门控（fail-closed）+配置诊断横幅 | 2 轮 | 0d6ce82 |
| #10 | 合规套件 4a：传输/流式+错误解析（7 情景+摘要 artifact） | 3 轮 | 452fa6f |
| #11 | 合规套件 4b：工具调用+多模态路由（4 情景+产品 slug 门修复） | 4 轮 | f604792 |
| #12 | 兼容矩阵联动（证据三分+固化审计链+落点钉版本） | 3 轮 | 74bfa69 |

产品级收获：图片门控 slug/key 解析次序（4b 套件抓出并修复）；
兼容性知识入册（引擎图片输入约束、image_description 配置契约、
5xx 重试预算）。证据长期审计链：docs/evidence/ 冻结摘要 + 双哈希 +
CI 校验器（scripts/verify_compliance_evidence.py）。

#127 关闭。下一步：v0.18.9 发布门（smoke + 发布流程）；#126 解冻待
单独设计。

## 2026-08-02 v0.18.9 发布门（全过）+ 生产更新器首次生产环境验收

七步序列：
1. 真实 API smoke 6/6（S1-S6，GLM+DeepSeek）
2. 治理专项冒烟 4/4：无 wancode.toml 空路径 / 损坏配置双链诊断 /
   能力热加载换代 / 图片转述+内联路由（caps_snapshot 6 测 + 4b 套件）
3. 升版构建 ed43fc4（msi 46776320B / setup 24755270B sha256 9d69c405…）
4. 防串线扫描：主 exe + setup 双查三痕迹全 absent
5. 签名发布 v0.18.9（4 资产）。事故与修复：latest.json 以 file#label
   语法上传导致资产名为 latest-189.json（updater 会 404）——发布后
   分钟级发现，删资产按正确文件名重传，端点实测解析 0.18.9
6. 双源验证：官方/镜像 sha 同一（9d69c405…）、MZ、minisign 双源 OK
7. 隔离安装官方 0.18.8（sha 45d2d44f…）→ 真实应用内升级 0.18.9：
   发现→"Downloading v0.18.9… 19%"帧→装前提示帧→退出安装自动重启
   （新 pid）→ exe 0.18.9；sessions 2070→2082 仅增、config sha 不变；
   注册表恢复、真实安装未动。**生产端点+生产密钥+正式资产的更新器
   全链首次生产环境验证通过。**
