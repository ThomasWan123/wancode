...
### v0.13.0 工程基础
- ☐ smoke.ps1 安全网（调整项，先于拆分）
- ☐ 拆 App.tsx / agent.rs（Codex 目录结构照单；每次提交只拆一域，拆后跑 smoke）
- ☐ invoke 集中到 ipc/commands.ts，消 any
- ☐ grok-build 固定 commit submodule；bootstrap.ps1；发布脚本去绝对路径
