...
| 指标 | 数值 |
|---|---|
| P2 启动前（2026-07-19 晨） | 52（29.2%） |
| **P2.1–P2.10 完成后（2026-07-19 晚）** | **117 = 65.4%** |
| 60% 门槛（108） | **已越过** ✅ |

P2 各批验证深度（如实分级）：
- **行为级**：P2.1 全部（插话当轮生效/队列编辑重排立即插话/模式引擎同步）、
  P2.2 主链路（toggle 写盘/重启后清单排除/内置技能可见）、P2.6 fuzzy
  （@rdm 子序列命中证明引擎流式通路）
- **通路级**（编译通过+契约核对，行为验证被阻塞或按时间预算跳过）：
  P2.3 MCP 配置、P2.4 终端补全（pty/load 重连）、P2.5 git 补全（stash 等，
  被下述 bug 阻塞）、P2.7 fs、P2.8 会话管理、P2.9 记忆杂项、P2.10 监听挂接
- **已知 bug（下一工作日头号）**：工作区切换后 git 面板可能显示前一仓库的
  改动（footer 标签来自 localStorage、会话归属另有其主，两者脱节）——
  此状态下 stash/丢弃会打错仓库，行为级验证因此暂停。
