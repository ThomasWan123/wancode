...
| 凭据与端点隔离 | 引擎层集成测试 | **引用引擎 Gate 1 测试**（正确端点 1 次、错误端点 0 次、Authorization 归属） | 引擎 [`tests/model_endpoint_routing.rs`](https://github.com/ThomasWan123/grok-build/blob/c562ae8130de35272c69ef6c83f1ff9cc0929e5f/crates/codegen/xai-grok-shell/tests/model_endpoint_routing.rs)（fork 固定 commit `c562ae81`，#126 B2 随模型身份域迁入；上游基线 `b189869b` 不含它）· CI 步"Gate 1 引擎侧路由证据" |
